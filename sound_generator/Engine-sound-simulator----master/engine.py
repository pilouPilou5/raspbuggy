...
class Engine:
    ...
    def throttle(self, fraction):
        '''Applique l'accélérateur et ajuste les RPM en fonction de la pression.'''
        if fraction > 0:
            # Courbe exponentielle pour une réponse plus réaliste
            acceleration_factor = (fraction ** 1.2) * (self.limiter_rpm - self.idle_rpm) * 0.05
            if self._rpm < self.limiter_rpm:
                self._rpm += acceleration_factor
                self._rpm = min(self._rpm, self.limiter_rpm)  # Limite les RPM au max du limiteur
        else:
            if self._rpm > self.idle_rpm:
                self._rpm -= 100  # Décélération graduelle
            else:
                self._rpm = self.idle_rpm
        print(f'RPM: {self._rpm:.0f}', end="\r")
